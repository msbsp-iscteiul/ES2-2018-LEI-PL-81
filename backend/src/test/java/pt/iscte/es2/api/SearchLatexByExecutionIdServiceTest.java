...
/**
 * Tests for the OptimizationConfiguration Service when requesting an Latex File by execution id
 */
public class SearchLatexByExecutionIdServiceTest {
	...
	public void successCall() {
		...
		Mockito.when(optimizationBusiness.searchLatexByExecutionId(Mockito.anyInt()))
			.thenReturn(new FileSystemResource(path.toFile()));
		Assert.assertNotNull(optimizationService.searchLatexByExecutionId(1));
	}
}
